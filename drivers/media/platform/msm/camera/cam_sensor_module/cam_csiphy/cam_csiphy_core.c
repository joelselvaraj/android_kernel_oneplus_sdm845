...
irqreturn_t cam_csiphy_irq(int irq_num, void *data)
{
	...
	for (i = 0; i < csiphy_dev->num_irq_registers; i++) {
		irq = cam_io_r_debug(base, base +
			...
			"csiphy_reg->mipi_csiphy_interrupt_status0_addr (0x%x) + (0x4 * %d)", csiphy_dev->ctrl_reg->csiphy_reg.mipi_csiphy_interrupt_status0_addr, i);
		cam_io_w_mb_debug(base, irq, base +
			csiphy_dev->ctrl_reg->csiphy_reg.mipi_csiphy_interrupt_clear0_addr + (0x4 * i),
			"csiphy_reg->mipi_csiphy_interrupt_clear0_addr (0x%x) + (0x4 * %d)", csiphy_dev->ctrl_reg->csiphy_reg.mipi_csiphy_interrupt_clear0_addr, i);
	...
	cam_io_w_mb_debug(base, 0x1, base + csiphy_dev->ctrl_reg->csiphy_reg.mipi_csiphy_glbl_irq_cmd_addr, "csiphy_reg->mipi_csiphy_glbl_irq_cmd_addr");
	cam_io_w_mb_debug(base, 0x0, base + csiphy_dev->ctrl_reg->csiphy_reg.mipi_csiphy_glbl_irq_cmd_addr, "csiphy_reg->mipi_csiphy_glbl_irq_cmd_addr");
